...
class OperationsService(operations_pb2_grpc.OperationsServicer):
    ...
    def GetOperation(self, request, context):
        self._logger.info(f"GetOperation request from [{context.peer()}]")
        ...
        try:
            ...
            operation, _ = instance.get_operation(operation_name)
            op = operations_pb2.Operation()
            op.CopyFrom(operation)
            op.name = name

            # TODO: Convert the metadata returned by `get_operation()`
            #  into a `RequestMetadata` message and attach it to gRPC
            #  trailing metadata as an entry of type:
            #  ('requestmetadata-bin', RequestMetadata).
    ...
    def ListOperations(self, request, context):
        self._logger.info(f"ListOperations request from [{context.peer()}]")
    ...
    def DeleteOperation(self, request, context):
        self._logger.info(f"DeleteOperation request from [{context.peer()}]")
    ...
    def CancelOperation(self, request, context):
        self._logger.info(f"CancelOperation request from [{context.peer()}]")
    ...
    @staticmethod
    def _parse_operation_name(name):
        names = name.split('/')
