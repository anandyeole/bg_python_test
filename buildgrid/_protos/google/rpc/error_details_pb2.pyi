"""
@generated by mypy-protobuf.  Do not edit manually!
isort:skip_file
"""
import builtins
import google.protobuf.descriptor
import google.protobuf.duration_pb2
import google.protobuf.internal.containers
import google.protobuf.message
import typing
import typing_extensions

DESCRIPTOR: google.protobuf.descriptor.FileDescriptor = ...

class RetryInfo(google.protobuf.message.Message):
    DESCRIPTOR: google.protobuf.descriptor.Descriptor = ...
    RETRY_DELAY_FIELD_NUMBER: builtins.int

    @property
    def retry_delay(self) -> google.protobuf.duration_pb2.Duration: ...

    def __init__(self,
        *,
        retry_delay : typing.Optional[google.protobuf.duration_pb2.Duration] = ...,
        ) -> None: ...
    def HasField(self, field_name: typing_extensions.Literal[u"retry_delay",b"retry_delay"]) -> builtins.bool: ...
    def ClearField(self, field_name: typing_extensions.Literal[u"retry_delay",b"retry_delay"]) -> None: ...
global___RetryInfo = RetryInfo

class DebugInfo(google.protobuf.message.Message):
    DESCRIPTOR: google.protobuf.descriptor.Descriptor = ...
    STACK_ENTRIES_FIELD_NUMBER: builtins.int
    DETAIL_FIELD_NUMBER: builtins.int
    stack_entries: google.protobuf.internal.containers.RepeatedScalarFieldContainer[typing.Text] = ...
    detail: typing.Text = ...

    def __init__(self,
        *,
        stack_entries : typing.Optional[typing.Iterable[typing.Text]] = ...,
        detail : typing.Text = ...,
        ) -> None: ...
    def ClearField(self, field_name: typing_extensions.Literal[u"detail",b"detail",u"stack_entries",b"stack_entries"]) -> None: ...
global___DebugInfo = DebugInfo

class QuotaFailure(google.protobuf.message.Message):
    DESCRIPTOR: google.protobuf.descriptor.Descriptor = ...
    class Violation(google.protobuf.message.Message):
        DESCRIPTOR: google.protobuf.descriptor.Descriptor = ...
        SUBJECT_FIELD_NUMBER: builtins.int
        DESCRIPTION_FIELD_NUMBER: builtins.int
        subject: typing.Text = ...
        description: typing.Text = ...

        def __init__(self,
            *,
            subject : typing.Text = ...,
            description : typing.Text = ...,
            ) -> None: ...
        def ClearField(self, field_name: typing_extensions.Literal[u"description",b"description",u"subject",b"subject"]) -> None: ...

    VIOLATIONS_FIELD_NUMBER: builtins.int

    @property
    def violations(self) -> google.protobuf.internal.containers.RepeatedCompositeFieldContainer[global___QuotaFailure.Violation]: ...

    def __init__(self,
        *,
        violations : typing.Optional[typing.Iterable[global___QuotaFailure.Violation]] = ...,
        ) -> None: ...
    def ClearField(self, field_name: typing_extensions.Literal[u"violations",b"violations"]) -> None: ...
global___QuotaFailure = QuotaFailure

class ErrorInfo(google.protobuf.message.Message):
    DESCRIPTOR: google.protobuf.descriptor.Descriptor = ...
    class MetadataEntry(google.protobuf.message.Message):
        DESCRIPTOR: google.protobuf.descriptor.Descriptor = ...
        KEY_FIELD_NUMBER: builtins.int
        VALUE_FIELD_NUMBER: builtins.int
        key: typing.Text = ...
        value: typing.Text = ...

        def __init__(self,
            *,
            key : typing.Text = ...,
            value : typing.Text = ...,
            ) -> None: ...
        def ClearField(self, field_name: typing_extensions.Literal[u"key",b"key",u"value",b"value"]) -> None: ...

    REASON_FIELD_NUMBER: builtins.int
    DOMAIN_FIELD_NUMBER: builtins.int
    METADATA_FIELD_NUMBER: builtins.int
    reason: typing.Text = ...
    domain: typing.Text = ...

    @property
    def metadata(self) -> google.protobuf.internal.containers.ScalarMap[typing.Text, typing.Text]: ...

    def __init__(self,
        *,
        reason : typing.Text = ...,
        domain : typing.Text = ...,
        metadata : typing.Optional[typing.Mapping[typing.Text, typing.Text]] = ...,
        ) -> None: ...
    def ClearField(self, field_name: typing_extensions.Literal[u"domain",b"domain",u"metadata",b"metadata",u"reason",b"reason"]) -> None: ...
global___ErrorInfo = ErrorInfo

class PreconditionFailure(google.protobuf.message.Message):
    DESCRIPTOR: google.protobuf.descriptor.Descriptor = ...
    class Violation(google.protobuf.message.Message):
        DESCRIPTOR: google.protobuf.descriptor.Descriptor = ...
        TYPE_FIELD_NUMBER: builtins.int
        SUBJECT_FIELD_NUMBER: builtins.int
        DESCRIPTION_FIELD_NUMBER: builtins.int
        type: typing.Text = ...
        subject: typing.Text = ...
        description: typing.Text = ...

        def __init__(self,
            *,
            type : typing.Text = ...,
            subject : typing.Text = ...,
            description : typing.Text = ...,
            ) -> None: ...
        def ClearField(self, field_name: typing_extensions.Literal[u"description",b"description",u"subject",b"subject",u"type",b"type"]) -> None: ...

    VIOLATIONS_FIELD_NUMBER: builtins.int

    @property
    def violations(self) -> google.protobuf.internal.containers.RepeatedCompositeFieldContainer[global___PreconditionFailure.Violation]: ...

    def __init__(self,
        *,
        violations : typing.Optional[typing.Iterable[global___PreconditionFailure.Violation]] = ...,
        ) -> None: ...
    def ClearField(self, field_name: typing_extensions.Literal[u"violations",b"violations"]) -> None: ...
global___PreconditionFailure = PreconditionFailure

class BadRequest(google.protobuf.message.Message):
    DESCRIPTOR: google.protobuf.descriptor.Descriptor = ...
    class FieldViolation(google.protobuf.message.Message):
        DESCRIPTOR: google.protobuf.descriptor.Descriptor = ...
        FIELD_FIELD_NUMBER: builtins.int
        DESCRIPTION_FIELD_NUMBER: builtins.int
        field: typing.Text = ...
        description: typing.Text = ...

        def __init__(self,
            *,
            field : typing.Text = ...,
            description : typing.Text = ...,
            ) -> None: ...
        def ClearField(self, field_name: typing_extensions.Literal[u"description",b"description",u"field",b"field"]) -> None: ...

    FIELD_VIOLATIONS_FIELD_NUMBER: builtins.int

    @property
    def field_violations(self) -> google.protobuf.internal.containers.RepeatedCompositeFieldContainer[global___BadRequest.FieldViolation]: ...

    def __init__(self,
        *,
        field_violations : typing.Optional[typing.Iterable[global___BadRequest.FieldViolation]] = ...,
        ) -> None: ...
    def ClearField(self, field_name: typing_extensions.Literal[u"field_violations",b"field_violations"]) -> None: ...
global___BadRequest = BadRequest

class RequestInfo(google.protobuf.message.Message):
    DESCRIPTOR: google.protobuf.descriptor.Descriptor = ...
    REQUEST_ID_FIELD_NUMBER: builtins.int
    SERVING_DATA_FIELD_NUMBER: builtins.int
    request_id: typing.Text = ...
    serving_data: typing.Text = ...

    def __init__(self,
        *,
        request_id : typing.Text = ...,
        serving_data : typing.Text = ...,
        ) -> None: ...
    def ClearField(self, field_name: typing_extensions.Literal[u"request_id",b"request_id",u"serving_data",b"serving_data"]) -> None: ...
global___RequestInfo = RequestInfo

class ResourceInfo(google.protobuf.message.Message):
    DESCRIPTOR: google.protobuf.descriptor.Descriptor = ...
    RESOURCE_TYPE_FIELD_NUMBER: builtins.int
    RESOURCE_NAME_FIELD_NUMBER: builtins.int
    OWNER_FIELD_NUMBER: builtins.int
    DESCRIPTION_FIELD_NUMBER: builtins.int
    resource_type: typing.Text = ...
    resource_name: typing.Text = ...
    owner: typing.Text = ...
    description: typing.Text = ...

    def __init__(self,
        *,
        resource_type : typing.Text = ...,
        resource_name : typing.Text = ...,
        owner : typing.Text = ...,
        description : typing.Text = ...,
        ) -> None: ...
    def ClearField(self, field_name: typing_extensions.Literal[u"description",b"description",u"owner",b"owner",u"resource_name",b"resource_name",u"resource_type",b"resource_type"]) -> None: ...
global___ResourceInfo = ResourceInfo

class Help(google.protobuf.message.Message):
    DESCRIPTOR: google.protobuf.descriptor.Descriptor = ...
    class Link(google.protobuf.message.Message):
        DESCRIPTOR: google.protobuf.descriptor.Descriptor = ...
        DESCRIPTION_FIELD_NUMBER: builtins.int
        URL_FIELD_NUMBER: builtins.int
        description: typing.Text = ...
        url: typing.Text = ...

        def __init__(self,
            *,
            description : typing.Text = ...,
            url : typing.Text = ...,
            ) -> None: ...
        def ClearField(self, field_name: typing_extensions.Literal[u"description",b"description",u"url",b"url"]) -> None: ...

    LINKS_FIELD_NUMBER: builtins.int

    @property
    def links(self) -> google.protobuf.internal.containers.RepeatedCompositeFieldContainer[global___Help.Link]: ...

    def __init__(self,
        *,
        links : typing.Optional[typing.Iterable[global___Help.Link]] = ...,
        ) -> None: ...
    def ClearField(self, field_name: typing_extensions.Literal[u"links",b"links"]) -> None: ...
global___Help = Help

class LocalizedMessage(google.protobuf.message.Message):
    DESCRIPTOR: google.protobuf.descriptor.Descriptor = ...
    LOCALE_FIELD_NUMBER: builtins.int
    MESSAGE_FIELD_NUMBER: builtins.int
    locale: typing.Text = ...
    message: typing.Text = ...

    def __init__(self,
        *,
        locale : typing.Text = ...,
        message : typing.Text = ...,
        ) -> None: ...
    def ClearField(self, field_name: typing_extensions.Literal[u"locale",b"locale",u"message",b"message"]) -> None: ...
global___LocalizedMessage = LocalizedMessage
